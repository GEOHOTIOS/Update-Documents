...
- 💥判断门关的标识统一使用tag
### 1.0.0
- 😄配置(chunk-config.[hash].js)附加参数中新增special参数 用于标识客户现场特殊配置
  ````javascript
    special: {
    vanke: {
      isGeneral: true, //是否部署在万科总部
      wechat: {
        link: 'http://t1.vanke.com:6888/ddl/#/authorize?devMac=', //员工扫码的链接地址
        route: 'ap.doc.wechat', //接受返回消息的吐送
      } //员工取件微信相关
    } //万科项目
  } //项目个性化配置
  ````
